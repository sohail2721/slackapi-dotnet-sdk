...
```csharp
AdminAppsRequestsController adminAppsRequestsController = client.AdminAppsRequestsController;
```
...
try
{
    ApiResponse<DefaultSuccessTemplate> result = await adminAppsRequestsController.AdminAppsRequestsListAsync(token);
}
