...
```csharp
AdminAppsRestrictedController adminAppsRestrictedController = client.AdminAppsRestrictedController;
```
...
try
{
    ApiResponse<DefaultSuccessTemplate> result = await adminAppsRestrictedController.AdminAppsRestrictedListAsync(token);
}
