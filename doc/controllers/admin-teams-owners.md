...
```csharp
AdminTeamsOwnersController adminTeamsOwnersController = client.AdminTeamsOwnersController;
```
...
try
{
    ApiResponse<DefaultSuccessTemplate> result = await adminTeamsOwnersController.AdminTeamsOwnersListAsync(
        token,
        teamId
    );
}
